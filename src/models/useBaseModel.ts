...
export const useBaseModel = <T>({ apiUrl, queryKey }: BaseModel): IBaseModelProps<T> => {
    ...
    const findOneById = <T>(id: string, queryKeys: string[] = queryKey): T => (queryClient.getQueryData(queryKeys) as [])?.find((item: any) => `${item.id}` === `${id}`) as T || {} as T

    const filterBy = <T>(filter: string, filterKey: string, queryKeys: string[] = queryKey): T => (queryClient.getQueryData(queryKeys) as [])?.filter((item: any) => item[filterKey]?.includes(filter)) as T || [] as T
    ...
    return { status, data, error, isFetching, isLoading, setLoading, hasError, findOneById, filterBy, $delete, $update, $save, refetch } as IBaseModelProps<T>
}
